...
tracking purposes, etc. Many facilities can be associated with a commercial facility, which are unique to your organization alone.
...
<ResponseField name="Organization" required>
  The organization that created this commercial facility (references `id` in
  Organization model)
</ResponseField>
...
<ResponseField name="Facilities">
  ...
  or Pipeline (references `id` in Pipeline model)
</ResponseField>
