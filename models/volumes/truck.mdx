...
<Info>
  ...
  multiple facilities under a single entry. While regulatory facilities
  (batteries, terminals, etc.) each have unique `regulator_id`'s, commercial
  facilities allow organizations to combine these into meaningful business units
  for marketing, stakeholder reporting, and internal tracking purposes. For
  example, multiple batteries with different `regulator_id`s might be grouped
  into one commercial facility that represents how the organization refers to
  that location internally.
</Info>
