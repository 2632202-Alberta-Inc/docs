...
3. Create the ticket via API with all required fields.
4. Get the ticket using the ticket ID and check the response for any validation errors.
5. Update ticket via API.
6. Confirm ticket has no errors and is in status `PENDING_REVIEW`.
...
2. **Validation**
   ...
   - Existence checks for all referenced entities (facilities, entiwells, meters, etc.)
