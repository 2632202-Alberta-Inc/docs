...
Each ticket type has specific required fields and validation rules. See the [ticket models](/concepts/volumes) for detailed specifications.
...
7. If no errors, status updates to `PENDING_REVIEW`.

<Tip>
  Note: Flux assigns a unique `id` across all organizations on the platform. The
  `external_id` field is specific to your organization and must remain unique
  for active tickets. This ensures that within each organization, only one
  active ticket can have a given `external_id` and attempting to create a
  duplicate active ticket will result in an error.
</Tip>
